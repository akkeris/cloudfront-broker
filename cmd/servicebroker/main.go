...
func runWithContext(ctx context.Context) error {
  ...
  if options.Insecure {
    ...
    if options.TLSCert != "" && options.TLSKey != "" {
      klog.V(4).Infof("Starting secure broker with TLS cert and key data")
      err = s.RunTLS(ctx, addr, options.TLSCert, options.TLSKey)
    } else {
      if options.TLSCertFile == "" || options.TLSKeyFile == "" {
        klog.Error("unable to run securely without TLS Certificate and Key. Please review options and if running with TLS, specify --tls-cert-file and --tls-private-key-file or --tlsCert and --tlsKey.")
        return nil
      }
      klog.V(4).Infof("Starting secure broker with file based TLS cert and key")
